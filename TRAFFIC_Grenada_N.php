...
<?php
	...
	from Indicadores_Temp where Pais='Grenada' and Hora between '0' and hour(now())+1";
	
	$sql2 = "select Min,Errores,Calls,ASR,ACD,NER from Indicadores_Actuales where Pais= 'Grenada'";
	...
	from VQAS3.Grenada B where date(Start_Time)=date(now()) and hour(Start_Time) between '0' and hour(now())+1 group by Region_Code order by Min desc limit 0,5";
	...
	from Indicadores_Temp_Region where Pais ='Grenada' and Hora between '0' and hour(now())+1
	group by Region order by Min desc limit 0,10";
	
	$sql5 = "select Last_Update LastUp,Min totMin from VQAS3.ASR_NER_Min_Temp where Pais='Grenada'";
...
<html>
  ...
  <body>
	<center><BR><BR><h2> Grenada Traffic Report</h2><br><table>
