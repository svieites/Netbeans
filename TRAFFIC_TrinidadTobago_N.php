...
<?php
	...
	from Indicadores_Temp where Pais='TrinidadTobago' and Hora between '0' and hour(now())+1";
	
	$sql2 = "select Min,Errores,Calls,ASR,ACD,NER from Indicadores_Actuales where Pais= 'TrinidadTobago'";
	...
	from VQAS3.TrinidadTobago B where date(Start_Time)=date(now()) and hour(Start_Time) between '0' and hour(now())+1 group by Region_Code order by Min desc limit 0,5";
	...
	from Indicadores_Temp_Region where Pais ='TrinidadTobago' and Hora between '0' and hour(now())+1
	group by Region order by Min desc limit 0,10";
	
	$sql5 = "select Last_Update LastUp,Min totMin from VQAS3.ASR_NER_Min_Temp where Pais='TrinidadTobago'";
	...
	$obs="";
	$obs.="<h2>Last Update (GMT-4): ".$last_update."<br/>\n<br></h2>";
	for ($i = 0; $i < 5; $i++) {
		$region_exists=false;
		for ($j = 0; $j < 10; $j++){
...
<html>
  ...
  <body>
	<center><BR><BR><h2> TrinidadTobago Traffic Report</h2><br><table>
	...
	<BR><CENTER><tr>
		<br><td>
			<div id="table_div"align="middle"style="width:750px"></div>
		</td>	
	</tr></CENTER>
