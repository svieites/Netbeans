...
?>
 <?php
	...
	AND Pais = 'Jamaica' AND Fecha BETWEEN  DATE(date_sub(sysdate(), interval 28 DAY)) AND DATE(SYSDATE()) ORDER BY DAYOFWEEK(FECHA),FECHA DESC";
    $sql2 ="select round((sum(Calls)-sum(Errors_ASR))*100/(sum(Calls))) Mean_ASR, round((sum(Calls)-sum(Errors_NER))*100/(sum(Calls))) Mean_NER from ASR_NER_Errors where dayname(Fecha) = dayname(now()) and Fecha between date(date_sub(sysdate(), interval 100 day)) and date(date_sub(sysdate(), interval 1 day)) and Pais = 'Jamaica'";
    $sql3 = "select ASR,NER,Last_Update from ASR_NER_Min_Temp where Pais = 'Jamaica'";
